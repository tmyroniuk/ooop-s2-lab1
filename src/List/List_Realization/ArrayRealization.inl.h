 ...
 * @author tmyroniuk
 * @date 14.10.2019
 * @brief Contains definition of ArrayRealization class methods.
 */
...
void ArrayRealization<T>::grow() {
    ...
    for (unsigned int i = 0; i < ArrayRealization<T>::len; i++) {
...
void ArrayRealization<T>::shrink() {
    if (ArrayRealization<T>::len != 0) return;
    auto temp = new Node;
...
void ArrayRealization<T>::insert(NodeIterator<T> *pos, T data) {
    ...
    i->setVal(data);
    ArrayRealization<T>::len++;
    if (ArrayRealization<T>::len + 1 >= reserved) grow();
}
...
NodeIterator<T> *ArrayRealization<T>::end() {
    return head + ArrayRealization<T>::len;
}
